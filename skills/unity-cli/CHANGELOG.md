...
Tracks the CLI's `1.0.0-beta.4` release. The skill's command coverage is still the `1.0.0-beta.3` surface: beta.4's additions — `unity skill install`/`refresh`, `unity projects clean`, `unity editors prune`/`verify`, `unity templates pack`, `unity bug --attachments`/`--share-project`, the `unity command` listing-query flags, `unity build --profile`, multi-account auth (`unity auth list`/`switch`/`default`), and the output pager — are **not** documented yet and land in the next skill pass. Documenting a subset of the shipped surface is safe; the stamp exists to stop the reverse (publishing surface that isn't in the shipped binary).
