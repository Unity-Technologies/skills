...
Tracks the CLI's `1.0.0-beta.4` release. Coverage is the full `1.0.0-beta.3` surface plus the beta.4 additions an automation or CI caller reaches for first: `unity test --report-format`/`--coverage`, `unity projects exec`, `unity bug --attachments`/`--share-project`, and the rule that a failure is readable from stdout. The rest of beta.4 lands in the next skill pass and is **not** documented here yet: `unity skill install`/`refresh`, `unity projects clean`, `unity editors prune`/`verify`, `unity templates pack`, the `unity command` listing-query flags, `unity build --profile`, multi-account auth (`unity auth list`/`switch`/`default`), and the output pager. Documenting a subset of the shipped surface is safe; the stamp exists to stop the reverse (publishing surface that isn't in the shipped binary).
...
- **`unity test --report-format nunit|junit`** (with `--junit-output`) — write a JUnit-schema report that GitHub Actions and GitLab ingest as native test results, with no converter step. `--report-format junit` makes `--output` the JUnit file; `nunit,junit` writes both from a single Editor run. The report is written even when tests fail, and the NUnit default is unchanged.
...
- **Read failures from stdout, not stderr** — documented the machine-format failure contract. Under `--format json` a failed command still writes a full envelope (`success: false`, `data: null`, a populated `errors` array whose `errors[0].code` is the stable token to branch on); under `--format ndjson` it closes with the usual terminal `result` frame. Empty stdout is not a failure signal, and the commands that still report only on stderr are called out as a known gap rather than a shape to code against.
...
- **`unity mcp configure --local`** — corrected the client list to `cursor`, `vscode`, `kiro`, and `codex`. Windsurf reads one global file and has no project-local variant.
