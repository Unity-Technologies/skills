...
`7` covers an Editor that is *running* but refuses the connection, not just a missing one: for example, your own sandbox refusing the loopback connection. If you are a sandboxed agent, read a `7` as "ask again once the sandbox allows loopback", never as a compile failure.
