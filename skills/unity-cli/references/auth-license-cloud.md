...
The interactive `unity auth login` flow prints the sign-in URL to the terminal **before** attempting to launch the browser, which unblocks remote/headless sessions (SSH, containers, dev VMs) where `xdg-open` / `open` has no graphical session to attach to. With `--format json`, an `auth_url=…` progress frame is emitted so machine consumers can capture the URL without parsing human text. A missing opener (e.g. no `xdg-open` on a minimal distro) is reported through the normal warning path and the command keeps waiting for the browser hand-off. Under WSL, URLs open through Windows interop (`wslview` when installed, falling back to `rundll32.exe`), so the sign-in page opens in the Windows browser on stock WSL Ubuntu. Cancel the wait with Ctrl-C (exit 130) or SIGTERM (exit 143) — both cancel the sign-in cleanly.
